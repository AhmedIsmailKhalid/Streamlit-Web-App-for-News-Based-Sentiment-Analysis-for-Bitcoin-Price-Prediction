...
def perform_eda() :  
    ...
    data_files_dir = [f for f in os.listdir(data_path) if os.path.isfile(os.path.join(data_path, f))]

    file = st.selectbox('Select File to Display', ['']+uploaded_files_dir+data_files_dir)
    
    if file in uploaded_files_dir :
        news_df =  pd.read_csv(os.path.join('uploaded data',file))
    
    if file in data_files_dir :
        news_df =  pd.read_csv(os.path.join('data',file))
